...
function focusOnLastQuantityField() {
    ...
    if (lastRow) {
        const lastBatchNumberField = lastRow.querySelector('.item-batchNumber');

        if (lastBatchNumberField) {
            lastBatchNumberField.focus();
        }
    }
}
...
function handleCloseButtonKeydown(event) {
    if (event.key === 'Enter') {
        ...
        // Focus on the batchNumber input field in the last row
